...
void update(int n, int b, int e, int i, int j, long long v) {
    ...
    if (lazy[n]) {
        seg[n] += lazy[n];
        if (b != e) lazy[2 * n] += lazy[n], lazy[2 * n + 1] += lazy[n];
        lazy[n] = 0;
    }
    if (b >= i && e <= j) {
        seg[n] += v;
        if (b != e) lazy[2 * n] += v, lazy[2 * n + 1] += v;
        return;
    }
    int mid = (b + e) / 2;
...
long long query(int n, int b, int e, int i, int j) {
    if (b > j || e < i) return LONG_LONG_MIN;
    if (lazy[n]) {
        seg[n] += lazy[n];
        if (b != e) lazy[2 * n] += lazy[n], lazy[2 * n + 1] += lazy[n];
        lazy[n] = 0;
...
void update(int B[], int x, int v) {
    while (x > 0) B[x] = min(B[x], v), x -= x & -x;
}

int query(int B[], int x) {
    int sum = INT_MAX;
    while (x < N) sum = min(sum, B[x]), x += x & -x;
    return sum;
}
...
int main() {
    ...
    cin.tie(0);
    cin >> n >> m >> p;
    for (int i = 0; i < N; i++) abits[i] = bbits[i] = INT_MAX;
    for (int i = 0; i < n; i++) {
    ...
    for (int i = 0; i < p; i++) { cin >> mons[i].x >> mons[i].y >> mons[i].z; }
    sort(mons, mons + p);
