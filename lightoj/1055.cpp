...
struct Point{
	...
	Point operator + (const Point &rhs) {
		...
		res.x = x + rhs.x;
		res.y = y + rhs.y;
		if(res.x < 0 || res.x >= n || res.y < 0 || res.y >= n || grid[res.x][res.y] == '#') return *this;
		return res;
	}
...
struct Pos {
	...
	bool reached(){
		...
		if(vec[0] == Point(dx[0], dy[0]) && vec[1] == Point(dx[1], dy[1]) && vec[2] == Point(dx[2], dy[2])) return true;
		return false;
		return true;
	}
...
int main(){
	...
	while(t--){
		...
		Pos begin = Pos(Point(sx[0], sy[0]), Point(sx[1], sy[1]), Point(sx[2], sy[2]));
		queue<Pos> q;
		q.push(begin);
		map<Pos, int> Dis;
		Dis[begin] = 0;
