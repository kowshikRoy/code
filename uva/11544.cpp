...
struct Edge {
  ...
  int main() {
    ...
    while (t--) {
      ...
      for (int i = 1; i <= m; i++) cin >> p[i], mf.addEdge(n + i, T, p[i]);
      ...
      for (int a = 1; a <= n; a++) {
        for (auto b : choice[a]) {
          ...
          p[b]--;
          for (int i = 1; i <= m; i++)
            if (p[i] > 0) mf.addEdge(n + i, T, p[i]);
          for (int i = a + 1; i <= n; i++) {
            mf.addEdge(0, i, 1);
            for (auto x : choice[i]) { mf.addEdge(i, n + x, 1); }
          }
      ...
      for (auto p : match) cout << p.first << " " << p.second << endl;
    }
  }
