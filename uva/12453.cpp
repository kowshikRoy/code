...
int main(){
    ...
    while(scanf("%lld %lld %lld %lld %lld",&s,&a,&b,&c,&d)) {
        ...
        while(true){
            ...
            cout << pivot << endl;
            if (pivot == 0) {
                A = A +  Pint(0,-s), B = B + Pint(0,-s), C = C + Pint(0,-s), D = D + Pint(0,-s);
                t = t - Pint(0, 2 * (t.y - A.y));
            }
            else if (pivot == 1) {
                A = A + Pint(s,0), B =B + Pint(s,0), C =C + Pint(s,0), D =D + Pint(s,0);
                t = t + Pint(2 * (B.x - t.x), 0);
            }
            else if( pivot == 2) {
                A = Pint(0,s), B = B + Pint(0,s), C =C+ Pint(0,s), D =D +   Pint(0,s);
                t = t + Pint(0, 2 * (D.y - t.y));
            }
            else if(pivot == 3) {
                A = Pint(-s,0), B =B + Pint(-s,0), C =C + Pint(-s,0), D =D + Pint(-s,0);
