...
bool IsLC(point a, point b, point c, point d) {
  return IsLP(a, b, c, d) && fabs(cross(a - b, a - c)) < EPS &&
...
bool SGIN(point a, point b, point c, point d) {
  if (IsLC(a, b, c, d)) {
    if (sqdist(a, c) < EPS || sqdist(a, d) < EPS || sqdist(b, c) < EPS ||
        sqdist(b, d) < EPS)
      return true;
...
bool POPoly(const vector<point> &p, point q) {
  for (int i = 0; i < p.size(); i++)
    if (sqdist(PPS(p[i], p[(i + 1) % p.size()], q), q) < EPS) return true;
  return false;
}
...
double TRICA(point r, point a, point b, double R) {
  ...
  return cross(ta - r, tb - r) / 2 + SectorArea(r, a, ta, R) +
         SectorArea(r, tb, b, R);
}
...
double SPICA(vector<point> &p, point r, double R) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (turn > 0)
      ret += TRICA(r, p[i], p[NEX(i)], R);
    else
      ret -= TRICA(r, p[NEX(i)], p[i], R);
  }
  return fabs(ret);
}
...
double CAR(const vector<point> &p) { return fabs(ComputeSignedArea(p)); }
...
bool IsSimple(const vector<point> &p) {
  for (int i = 0; i < p.size(); i++) {
    for (int k = i + 1; k < p.size(); k++) {
      ...
      int l = (k + 1) % p.size();
      if (i == l || j == k) continue;
      if (SGIN(p[i], p[j], p[k], p[l])) return false;
    }
  }
  return true;
}
...
int main() {
  long long s, a, b, c, d;
  int cs = 0;
  while (scanf("%lld %lld %lld %lld %lld", &s, &a, &b, &c, &d)) {
    if (s == 0) break;
    Pint X = Pint(a, b);
    Pint Y = Pint(c, d);
    Pint A = Pint(0, 0);
    ...
    while (true) {
      ...
      for (int i = 0; i < vectors.size(); i++) {
        vec vector = vectors[i];
        if (dot(ray, vector) > 0 && cw(Pint(0, 0), vector, ray)) { pivot = i; }
      }
      cout << pivot << endl;
      if (pivot == 0) {
        A = A + Pint(0, -s), B = B + Pint(0, -s), C = C + Pint(0, -s),
        D = D + Pint(0, -s);
        t = t - Pint(0, 2 * (t.y - A.y));
      } else if (pivot == 1) {
        A = A + Pint(s, 0), B = B + Pint(s, 0), C = C + Pint(s, 0),
        D = D + Pint(s, 0);
        t = t + Pint(2 * (B.x - t.x), 0);
      } else if (pivot == 2) {
        A = Pint(0, s), B = B + Pint(0, s), C = C + Pint(0, s),
        D = D + Pint(0, s);
        t = t + Pint(0, 2 * (D.y - t.y));
      } else if (pivot == 3) {
        A = Pint(-s, 0), B = B + Pint(-s, 0), C = C + Pint(-s, 0),
        D = D + Pint(-s, 0);
        t = t - Pint(2 * (t.x - A.x), 0);
      }
      ans++;

      cout << t.x << " " << t.y << endl;

      if (dot(t - origin, ray) == 0) break;
      if (ans > 10) break;
    }
    printf("Case %d: %d\n", ++cs, ans);
  }
