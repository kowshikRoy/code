...
node *reassign(node *n, node *l, node *r, unsigned int pri, int sz,
               int height, int val) {
  n->left = l;
  n->right = r;
  n->pri = pri;
...
node *reassign(node *n, node *l, node *r) {
  return reassign(n, l, r, n->pri, 1 + (l ? l->sz : 0) + (r ? r->sz : 0), n->height,
...
int main() {
  ...
  while (q--) {
    ...
    if (type == 'A') {
      node *cur = reassign(new node(), nullptr, nullptr, genPri(), 1, x,x);
      if (!root)
        root = cur;
      else
        root = ins(root, cur, y);
